        ...
        <div class="ip-tag-pop">
          ...
          <div class="ip-tag-pop__body">
            <template v-for="group in groups" :key="group.key">
              <div class="ip-tag-pop__group">
                <i class="ip-tag-dot" :class="`is-${group.key}`"></i>
                {{ group.label }} · {{ group.items.length }}
              </div>
              <div v-for="(item, index) in group.items" :key="`${group.key}-${index}`" class="ip-tag-pop__row">
...
function categoryOf(tag: string) {
...
const restCount = computed(() => Math.max(0, sortedTags.value.length - 1));
const totalCnt = computed(() => sortedTags.value.reduce((sum, item) => sum + item.cnt, 0));
const maxCnt = computed(() => sortedTags.value.reduce((max, item) => Math.max(max, item.cnt), 0));

const groups = computed(() => {
  const bucket: Record<string, CatTag[]> = {};
  sortedTags.value.forEach((item) => {
    if (!bucket[item.cat]) bucket[item.cat] = [];
    bucket[item.cat].push(item);
  });
  return CATEGORY_ORDER.filter((key) => bucket[key]).map((key) => ({
    key,
    label: t(`dashboard.ip_rank.tag_cat_${key}`),
    items: bucket[key],
  }));
});

function barWidth(item: CatTag) {
  if (!maxCnt.value) return '0%';
  return `${Math.max(4, Math.round((item.cnt / maxCnt.value) * 100))}%`;
}
...
.ip-tag-pop__bar i.is-attack {
  background: var(--td-error-color);
}
.ip-tag-pop__bar i.is-owasp {
  background: var(--td-brand-color);
}
.ip-tag-pop__bar i.is-list {
  background: var(--td-warning-color);
}
.ip-tag-pop__bar i.is-pass {
  background: var(--td-success-color);
}
.ip-tag-pop__bar i.is-other {
  background: var(--td-text-color-placeholder);
}
