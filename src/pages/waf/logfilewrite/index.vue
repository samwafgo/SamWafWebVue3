  ...
  <div>
            ...
            <t-col :span="6">
              <t-form-item :label="t('page.logfilewrite.max_backups')" name="max_backups">
                <t-input-number
                  ...
                  :max="100"
                  theme="normal"
                  :suffix="t('page.logfilewrite.unit_files')"
                />
              </t-form-item>
            </t-col>
          </t-row>
            ...
            <t-col :span="6">
              <t-form-item :label="t('page.logfilewrite.max_days')" name="max_days">
                <t-input-number
                  ...
                  :max="365"
                  theme="normal"
                  :suffix="t('page.logfilewrite.unit_days')"
                />
              </t-form-item>
            </t-col>
            <t-col :span="6">
    ...
    <t-card class="list-card-container">
      ...
      <t-loading :loading="previewLoading">
        <div v-if="logPreviewLines.length > 0" class="log-preview-box">
    ...
    <t-card class="list-card-container">
      ...
      <t-loading :loading="backupLoading">
        <t-table v-if="backupFiles.length > 0" :columns="backupColumns" :data="backupFiles" row-key="name" hover :pagination="undefined">
          <template #size="{ row }">
...
} from '@/apis/logfilewrite';

const { t } = useI18n();

// nginx combined 默认模板
const NGINX_TEMPLATE =
  '${src_ip} - - [${create_time}] "${method} ${url} HTTP/1.1" ${status_code} ${content_length} "${referer}" "${user_agent}"';
...
const logPreviewLines = ref<string[]>([]);
const currentFileInfo = ref<Record<string, any> | null>(null);
const backupFiles = ref<Record<string, any>[]>([]);

const variableColumns = computed<TableProps['columns']>(() => [
  { colKey: 'name', title: t('page.logfilewrite.var_name'), width: 200 },
  { colKey: 'desc', title: t('page.logfilewrite.var_desc'), width: 150 },
  { colKey: 'example', title: t('page.logfilewrite.var_example'), width: 250 },
]);

const backupColumns = computed<TableProps['columns']>(() => [
  { colKey: 'name', title: t('page.logfilewrite.col_filename'), ellipsis: true },
  { colKey: 'size', title: t('page.logfilewrite.col_size'), width: 120 },
  { colKey: 'mod_time', title: t('page.logfilewrite.col_time'), width: 200 },
]);

const computedPreview = computed(() => {
  let tpl = '';
  if (formData.value.format === 'custom') {
    tpl = formData.value.custom_tpl || NGINX_TEMPLATE;
  } else {
    tpl = NGINX_TEMPLATE;
  }
  // 用示例值替换模板变量
...
function fetchData() {
  ...
  const promises = configItems.map((item) => get_detail_by_item_api({ item }).catch(() => null));
        ...
        if (res && res.code === 0 && res.data && res.data.value !== undefined) {
          ...
          switch (item) {
            case 'log_file_write_enable':
              formData.value.enable = val === '1';
              break;
            case 'log_file_write_path':
              formData.value.file_path = val || 'logs/access.log';
              break;
            case 'log_file_write_format':
              formData.value.format = val || 'nginx';
              break;
            case 'log_file_write_custom_tpl':
              formData.value.custom_tpl = val || '';
              break;
            case 'log_file_write_max_size':
              formData.value.max_size = parseInt(val, 10) || 100;
              break;
            case 'log_file_write_max_backups':
              formData.value.max_backups = parseInt(val, 10) || 10;
              break;
            case 'log_file_write_max_days':
              formData.value.max_days = parseInt(val, 10) || 30;
              break;
            case 'log_file_write_compress':
              formData.value.compress = val === '1';
              break;
            default:
              break;
          }
...
async function handleSave() {
  ...
  try {
    const configItems = [
      { item: 'log_file_write_enable', value: formData.value.enable ? '1' : '0' },
      { item: 'log_file_write_path', value: formData.value.file_path },
      { item: 'log_file_write_format', value: formData.value.format },
      { item: 'log_file_write_custom_tpl', value: formData.value.custom_tpl },
      { item: 'log_file_write_max_size', value: String(formData.value.max_size) },
      { item: 'log_file_write_max_backups', value: String(formData.value.max_backups) },
      { item: 'log_file_write_max_days', value: String(formData.value.max_days) },
      { item: 'log_file_write_compress', value: formData.value.compress ? '1' : '0' },
    ];
...
function fetchPreview() {
  ...
  Promise.all([getLogPreviewApi({ lines: 100 }), getCurrentFileInfoApi()])
    .then(([previewRes, fileInfoRes]) => {
      if (previewRes.code === 0 && previewRes.data) {
        logPreviewLines.value = previewRes.data;
      }
      if (fileInfoRes.code === 0 && fileInfoRes.data) {
        currentFileInfo.value = fileInfoRes.data;
      }
    })
    .catch(() => {})
    .finally(() => {
...
function handleClearLog() {
  clearLogFileApi()
    .then((res) => {
      if (res.code === 0) {
        MessagePlugin.success(t('page.logfilewrite.clear_success'));
        fetchPreview();
      } else {
        MessagePlugin.error(res.msg || t('page.logfilewrite.clear_failed'));
      }
    })
    .catch(() => {
      MessagePlugin.error(t('page.logfilewrite.clear_failed'));
    });
}
...
function copyVariable(variable: string) {
  if (navigator.clipboard) {
    navigator.clipboard.writeText(variable).then(() => {
      MessagePlugin.success(`${t('page.logfilewrite.copy_success')}: ${variable}`);
    });
  } else {
    // fallback
