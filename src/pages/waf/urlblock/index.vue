<template>
  <div>
    ...
    <t-dialog
      ...
      :header="t('page.urlblock.confirm_batch_delete')"
    ...
    <t-dialog
      ...
      :header="t('page.urlblock.confirm_clear_all')"
...
const rules: FormProps['rules'] = {
  host_code: [{ required: true, message: t('common.select_placeholder') + t('page.urlblock.label_website'), type: 'error' }],
  compare_type: [{ required: true, message: t('common.select_placeholder'), type: 'error' }],
  url: [{ required: true, message: t('common.select_placeholder') + t('page.urlblock.label_url'), type: 'error' }],
};

const compareTypeOptions = computed(() => [
  { label: t('page.urlblock.compare_type_option_equal'), value: '等于' },
  { label: t('page.urlblock.compare_type_option_pre'), value: '前缀匹配' },
  { label: t('page.urlblock.compare_type_option_end'), value: '后缀匹配' },
  { label: t('page.urlblock.compare_type_option_contain'), value: '包含匹配' },
]);
...
const columns = computed<TableProps['columns']>(() => [
  ...
  { title: t('page.urlblock.label_website'), align: 'left', width: 250, ellipsis: true, colKey: 'host_code' },
  { title: t('page.urlblock.label_compare_type'), align: 'left', width: 250, ellipsis: true, colKey: 'compare_type' },
  { title: t('page.urlblock.label_url'), width: 200, ellipsis: true, colKey: 'url' },
...
const onSubmit: FormProps['onSubmit'] = ({ firstError }) => {
  if (!firstError) {
    wafURLBlockAddApi({ ...formData.value })
      .then((res) => {
        if (res.code === 0) {
...
const onSubmitEdit: FormProps['onSubmit'] = ({ firstError }) => {
  if (!firstError) {
    wafURLBlockEditApi({ ...formEditData.value })
      .then((res) => {
        if (res.code === 0) {
          MessagePlugin.success(res.msg);
...
function handleBatchDelete() {
  if (selectedRowKeys.value.length === 0) {
    MessagePlugin.warning(t('page.urlblock.no_data_selected'));
    return;
  }
...
function onConfirmBatchDelete() {
  ...
  wafURLBlockBatchDelApi({ ids })
    .then((res) => {
      if (res.code === 0) {
        getList();
        selectedRowKeys.value = [];
        MessagePlugin.success(t('page.urlblock.batch_delete_success'));
      } else {
        MessagePlugin.warning(res.msg);
      }
    })
...
function onConfirmClearAll() {
  ...
  wafURLBlockDelAllApi({ host_code: searchformData.host_code })
    .then((res) => {
      if (res.code === 0) {
        getList();
        selectedRowKeys.value = [];
        MessagePlugin.success(t('page.urlblock.clear_all_success'));
      } else {
        MessagePlugin.warning(res.msg);
      }
    })
