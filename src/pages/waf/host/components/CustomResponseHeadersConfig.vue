<template>
  <div class="custom-response-headers-config">
    ...
    <t-form-item :label="t('page.host.custom_response_headers.is_enable')">
      <t-tooltip :content="t('page.host.custom_response_headers.is_enable_tips')" placement="top" :overlay-style="{ width: '300px' }" show-arrow>
    ...
    <template v-if="localConfig.is_enable_custom_headers == '1'">
      <!-- 使用说明 -->
      <t-alert theme="info" :message="t('page.host.custom_response_headers.usage_guide')" style="margin-bottom: 16px" />
      ...
      <div v-else class="rules-container">
        <div v-for="(rule, ruleIndex) in localConfig.rules" :key="ruleIndex" class="rule-card">
          <!-- 规则卡片头部 -->
          <div class="rule-card-header">
            ...
            <t-button theme="danger" size="small" variant="outline" @click="removeRule(ruleIndex)">
          ...
          <div class="rule-card-body">
            <div class="rule-fields-row">
              ...
              <div class="rule-field">
                ...
                <t-select v-model="rule.match_type" style="width: 160px" @change="onMatchTypeChange(ruleIndex)">
              ...
              <div v-if="rule.match_type && rule.match_type !== 'global'" class="rule-field rule-field-grow">
                ...
                <t-input v-model="rule.match_value" :placeholder="getMatchValuePlaceholder(rule.match_type)" style="width: 260px" @change="updateParent" />
              </div>
              ...
              <div v-if="rule.match_type && rule.match_type !== 'global'" class="rule-field">
                ...
                <t-select v-model="rule.merge_mode" style="width: 220px" @change="updateParent">
            ...
            <div class="headers-section">
              ...
              <div v-if="!rule.headers || rule.headers.length === 0" class="empty-hint-small">
                <info-circle-icon style="margin-right: 6px" />
                {{ t('page.host.custom_response_headers.no_rule_headers') }}
              </div>

              <div v-else class="headers-container">
                <div v-for="(header, headerIndex) in rule.headers" :key="headerIndex" class="header-item">
                  <div class="header-item-row">
                    ...
                    <div class="header-field header-value-field">
                      ...
                      >
                        <template #suffix>
                          <t-dropdown :options="variableOptions" @click="(data: any) => insertVariable(ruleIndex, headerIndex, String(data.value))">
                    ...
                    <t-button theme="danger" size="small" variant="outline" class="delete-btn" @click="removeHeader(ruleIndex, headerIndex)">
                      <delete-icon style="margin-right: 4px" />
                      {{ t('common.delete') }}
                    </t-button>
                  </div>
...
const props = defineProps<{ customResponseHeadersConfig: Record<string, any> }>();
...
const localConfig = ref<Record<string, any>>(normalizeConfig(JSON.parse(JSON.stringify(props.customResponseHeadersConfig))));

watch(
  () => props.customResponseHeadersConfig,
  (newVal) => {
    localConfig.value = normalizeConfig(JSON.parse(JSON.stringify(newVal)));
    // 若旧格式 headers 被转为 rules，主动通知父组件同步为新格式，避免保存时 rules 丢失
...
function getMatchTypeLabel(matchType: string) {
  const map: Record<string, string> = {
    global: t('page.host.custom_response_headers.match_type_global'),
    prefix: t('page.host.custom_response_headers.match_type_prefix'),
    suffix: t('page.host.custom_response_headers.match_type_suffix'),
    exact: t('page.host.custom_response_headers.match_type_exact'),
    regex: t('page.host.custom_response_headers.match_type_regex'),
  };
  return map[matchType] || map['global'];
}

function getMatchValuePlaceholder(matchType: string) {
  const map: Record<string, string> = {
    prefix: t('page.host.custom_response_headers.match_value_placeholder_prefix'),
    suffix: t('page.host.custom_response_headers.match_value_placeholder_suffix'),
    exact: t('page.host.custom_response_headers.match_value_placeholder_exact'),
    regex: t('page.host.custom_response_headers.match_value_placeholder_regex'),
  };
  return map[matchType] || t('page.host.custom_response_headers.match_value_placeholder_default');
}

function onMatchTypeChange(ruleIndex: number) {
  // 切换到 global 时清空 match_value，切换到其他类型时设默认 merge_mode
  const rule = localConfig.value.rules[ruleIndex];
...
function removeRule(ruleIndex: number) {
  localConfig.value.rules.splice(ruleIndex, 1);
  updateParent();
}
...
function removeHeader(ruleIndex: number, headerIndex: number) {
  localConfig.value.rules[ruleIndex].headers.splice(headerIndex, 1);
  updateParent();
}

function insertVariable(ruleIndex: number, headerIndex: number, variable: string) {
  const header = localConfig.value.rules[ruleIndex].headers[headerIndex];
  if (header) {
...
/** 快速添加预设规则 */
function addPresetRule(type: string) {
  if (!localConfig.value.rules) {
    localConfig.value.rules = [];
  }
  let newRule: Record<string, any>;
  switch (type) {
  ...
  updateParent();
  MessagePlugin.success(t('page.host.custom_response_headers.add_rule_success'));
}
</script>
