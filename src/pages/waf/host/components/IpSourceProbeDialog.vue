<template>
  ...
  <t-dialog :header="dialogTitle" :visible="visible" :width="900" :footer="false" @update:visible="(v: boolean) => emit('update:visible', v)">
    ...
    <div class="ip-probe-toolbar">
      <div v-if="probeData" class="ip-probe-summary">
        ...
        <span>
          ...
          <b :style="{ color: probeData.compat_header_unset ? 'var(--td-error-color)' : '' }">
            {{
              probeData.effective_header ||
              (probeData.compat_header_unset ? t('page.host.ip_probe_effective_header_unset') : t('page.host.ip_probe_effective_header_none'))
            }}
          </b>
        </span>
    ...
    <div v-for="(sample, idx) in (probeData && probeData.samples) || []" :key="idx" class="ip-probe-sample">
      ...
      <div class="ip-probe-line">
        ...
        <t-tag
          ...
          size="small"
          variant="light"
          :theme="sample.net_ip_trusted ? 'success' : 'danger'"
      ...
      <div
        v-for="(tip, ti) in sampleTips(sample)"
        :key="'tip' + ti"
        :class="['ip-probe-tip', tip.type === 'ok' ? 'ip-probe-tip--ok' : 'ip-probe-tip--warn']"
      >
        {{ tip.text }}
      </div>
        ...
        <tbody>
          <tr v-for="(header, hi) in sample.headers" :key="hi" :class="{ 'ip-probe-row--ip': header.is_ip_header }">
            ...
            <td v-if="canUseHeader">
              <a v-if="header.parsed_ip" class="t-button-link" @click="emit('use-header', header.name)">
                {{ t('page.host.ip_probe_use_header') }}
              </a>
            </td>
          </tr>
        </tbody>
...
const props = withDefaults(
  ...
  { hostCode: '', hostName: '', canUseHeader: false },
);
const emit = defineEmits<{
  (e: 'update:visible', v: boolean): void;
  (e: 'use-header', name: string): void;
}>();
...
const probeData = ref<Record<string, any> | null>(null);

const dialogTitle = computed(() => `${t('page.host.ip_probe_title')}${props.hostName ? ` · ${props.hostName}` : ''}`);
...
const modeText = computed(() => {
  ...
  const mode = probeData.value?.ip_source_mode || '';
  return t(map[mode] || 'page.host.ip_source_compat');
});

// 只有会做来源校验的模式，标"来源可信/不可信"才有意义
const needTrustCheck = computed(() => {
  if (!probeData.value) return false;
  if (probeData.value.ip_source_mode === 'cdn_preset') return true;
  return ['header', 'xff_depth'].includes(probeData.value.ip_source_mode) && !!(probeData.value.ip_trust_proxies || '').trim();
});

function loadProbe() {
  if (!props.hostCode) return;
  loading.value = true;
  getIpSourceProbe({ code: props.hostCode })
    .then((res) => {
      if (res.code === 0) {
        probeData.value = res.data;
      } else {
...
function clearProbe() {
  if (!props.hostCode) return;
  clearIpSourceProbe({ code: props.hostCode })
    .then(() => {
...
function toggleProbe(enable: boolean) {
  ...
  get_detail_by_item_api({ item: 'ipprobe_enable' })
    .then((res) => {
      ...
      return edit_system_config_api({ ...res.data, value: enable ? '1' : '0' });
    })
    .then((res: any) => {
      if (!res) return;
      if (res.code === 0) {
        MessagePlugin.success(t(enable ? 'page.host.ip_probe_enable_ok' : 'page.host.ip_probe_disable_ok'));
        loadProbe();
      } else {
        MessagePlugin.error(res.msg);
      }
...
function sampleTips(sample: Record<string, any>) {
  const tips: Array<{ type: string; text: string }> = [];
  const data = probeData.value || ({} as Record<string, any>);
  // 兼容模式最容易踩的坑：全局「获取访客IP头信息」没配，等于谁都不读，永远取网络层IP
  if (data.compat_header_unset) {
    tips.push({ type: 'warn', text: t('page.host.ip_probe_tip_compat_unset') });
  }
  if (data.ip_source_mode === 'cdn_preset' && !sample.net_ip_trusted) {
    tips.push({ type: 'warn', text: t('page.host.ip_probe_tip_untrusted') });
  }
  if (data.effective_header && !sample.config_header_present) {
    tips.push({ type: 'warn', text: t('page.host.ip_probe_tip_header_missing', { header: data.effective_header }) });
  }
  if (sample.fallback) {
    tips.push({ type: 'warn', text: t('page.host.ip_probe_tip_fallback') });
  }
  if (tips.length === 0) {
    tips.push({ type: 'ok', text: t('page.host.ip_probe_tip_ok') });
  }
  return tips;
}
