<template>
  <t-dialog
    :visible="visible"
    :header="t('page.ccrule.emg_title')"
    :width="720"
    :footer="false"
    destroy-on-close
  ...
  >
    ...
    <div v-for="row in activeList" :key="row.host_code" class="emg-row">
      <span class="n">{{ row.global_host === 1 ? t('page.ccrule.emg_global_site') : row.host_name }}</span>
      <span class="tm">{{ untilText(row) }}</span>
...
const props = withDefaults(
  defineProps<{ visible: boolean; hosts?: Record<string, any>[]; defaultHostCode?: string }>(),
  { hosts: () => [], defaultHostCode: '' },
);
...
const activeList = ref<Record<string, any>[]>([]);
...
function loadStatus() {
  ...
  wafAntiCCRuleEmergencyStatusApi({})
    .then((res: any) => {
      loading.value = false;
      if (res.code === 0) activeList.value = res.data || [];
    })
    .catch(() => {
      loading.value = false;
    });
...
function turnOn() {
  saving.value = true;
  wafAntiCCRuleSetEmergencyApi({ host_code: hostCode.value, enable: 1, duration_min: durationMin.value })
    .then((res: any) => {
      saving.value = false;
      if (res.code === 0) {
...
function untilText(row: Record<string, any>) {
  if (!row.until) return t('page.ccrule.emg_until_manual');
  ...
  if (!row.active) return t('page.ccrule.emg_until_expired', { time });
  return t('page.ccrule.emg_until_at', { time });
}
...
watch(
  ...
  (val) => {
    ...
    hostCode.value = props.defaultHostCode || (props.hosts[0] && props.hosts[0].value) || '';
