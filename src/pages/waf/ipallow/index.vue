<template>
  <div>
      ...
      <t-row justify="space-between">
        <div class="left-operation-container">
          ...
          <t-button theme="danger" variant="outline" :disabled="selectedRowKeys.length === 0" @click="handleBatchDelete">
            {{ t('page.ipallow.button_batch_delete') }}
          </t-button>
    ...
    <t-dialog
      ...
      :header="t('page.ipallow.confirm_batch_delete')"
      @confirm="onConfirmBatchDelete"
      @cancel="batchDeleteVisible = false"
    >
      {{ t('page.ipallow.confirm_batch_delete') }}
    </t-dialog>

    <t-dialog
      v-model:visible="clearAllVisible"
      :header="t('page.ipallow.confirm_clear_all')"
      @confirm="onConfirmClearAll"
      @cancel="clearAllVisible = false"
    >
      {{ t('page.ipallow.confirm_clear_all') }}
    </t-dialog>
  </div>
</template>
...
const onSubmit: FormProps['onSubmit'] = ({ firstError }) => {
  if (!firstError) {
    wafIPWhiteAddApi({ ...formData.value })
      .then((res) => {
        if (res.code === 0) {
...
const onSubmitEdit: FormProps['onSubmit'] = ({ firstError }) => {
  if (!firstError) {
    wafIPWhiteEditApi({ ...formEditData.value })
      .then((res) => {
        if (res.code === 0) {
          MessagePlugin.success(res.msg);
...
function handleJumpOnlineUrl() {
  window.open(`${getOnlineUrl()}/guide/IPWhite.html`);
}

function handleBatchDelete() {
  if (selectedRowKeys.value.length === 0) {
    MessagePlugin.warning(t('page.ipallow.no_data_selected'));
    return;
  }
...
function onConfirmBatchDelete() {
  ...
  wafIPWhiteBatchDelApi({ ids })
    .then((res) => {
      if (res.code === 0) {
        getList();
        selectedRowKeys.value = [];
        MessagePlugin.success(t('page.ipallow.batch_delete_success'));
      } else {
        MessagePlugin.warning(res.msg);
      }
    })
...
function onConfirmClearAll() {
  ...
  wafIPWhiteDelAllApi({ host_code: searchformData.host_code })
    .then((res) => {
      if (res.code === 0) {
        getList();
        selectedRowKeys.value = [];
        MessagePlugin.success(t('page.ipallow.clear_all_success'));
      } else {
        MessagePlugin.warning(res.msg);
      }
    })
