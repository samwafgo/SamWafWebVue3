  ...
  <div>
    ...
    <t-alert theme="success" style="margin-top: 8px">
      <template #message>
        ...
        <t-button variant="text" theme="success" size="small" style="padding: 0 4px" @click="gotoUrlWhitelist">{{
          t('page.owasp.tuning.goto_url_whitelist')
        }}</t-button>
        <span style="color: var(--td-text-color-secondary)">|</span>
        <t-button variant="text" theme="success" size="small" style="padding: 0 4px" @click="gotoIpWhitelist">{{
          t('page.owasp.tuning.goto_ip_whitelist')
        }}</t-button>
      </template>
    </t-alert>

    <!-- 概念说明区：帮助用户理解 Paranoia / Anomaly 等核心术语 -->
    <t-collapse :default-value="['concepts']" expand-mutex expand-icon-placement="right" class="concept-panel">
    ...
    <t-form :data="form" :label-width="220" @submit="onSubmit">
      ...
      <t-form-item>
        ...
        <t-switch v-model="earlyBool" @change="(v: any) => (form.early_blocking = v ? 1 : 0)" />
    ...
    <div class="crs-vars-section">
      ...
      <t-collapse :default-value="[]" expand-icon-placement="right" style="margin-bottom: 12px">
        <t-collapse-panel :header="t('page.owasp.tuning.crs_vars_known_title')" value="known">
          <t-table :columns="knownVarColumns" :data="knownVars" row-key="key" size="small">
            <template #action="{ row }">
      ...
      <t-loading :loading="crsVarsLoading">
        <t-table :columns="crsVarColumns" :data="crsVarRows" row-key="key" size="small" :empty="t('page.owasp.tuning.crs_vars_empty')">
          <template #value="{ row }">
    ...
    <!-- 新增/编辑变量弹窗 -->
    <t-dialog
      v-model:visible="editVarDialog.visible"
      ...
      :on-confirm="onSaveVar"
      :on-close="() => (editVarDialog.visible = false)"
    >
      <t-form :data="editVarDialog" label-align="right" :label-width="120">
        <t-form-item :label="t('page.owasp.tuning.crs_var_key')">
          <t-input v-model="editVarDialog.key" :disabled="!editVarDialog.isNew" placeholder="allowed_methods" style="width: 280px" />
...
import { owaspCRSVarDeleteApi, owaspCRSVarSetApi, owaspCRSVarsGetApi, owaspTuningGetApi, owaspTuningSetApi } from '@/apis/owasp';
...
const KNOWN_CRS_VARS = [
  ...
  // input 被包裹为 |mime-type|，列表每项也必须用 |...| 包裹，项间空格分隔
  {
    ...
    default:
      '|application/x-www-form-urlencoded| |multipart/form-data| |multipart/related| |text/xml| |application/xml| |application/soap+xml| |application/json| |application/ld+json| |application/x-json| |text/plain| |application/octet-stream|',
    desc: '允许的请求 Content-Type（每项用 | 包裹，项间空格分隔）',
    rules: '920420',
  },
  // input 被包裹为 |charset|，列表每项也必须用 |...| 包裹
  {
    key: 'allowed_request_content_type_charset',
    default: '|utf-8| |utf-16| |iso-8859-1| |iso-8859-15| |windows-1252| |us-ascii| |gbk| |gb2312| |big5|',
    desc: '允许的 Content-Type 字符集（每项用 | 包裹，项间空格分隔）',
    rules: '920480',
  },
  // input 不包裹，空格分隔即可
  {
...
const earlyBool = ref(false);

const bodyLimitPresets = [
  { label: '默认', value: 0 },
...
const bodyInMemoryLimitPresets = [
  { label: '默认', value: 0 },
...
// CRS 变量
const crsVarsLoading = ref(false);
const crsVars = ref<Record<string, string>>({});
const editVarDialog = reactive({
  visible: false,
  isNew: true,
  saving: false,
  key: '',
...
const knownVarColumns = computed<TableProps['columns']>(() => [
...
const crsVarColumns = computed<TableProps['columns']>(() => [
  { title: '变量（tx.）', colKey: 'key', width: 260 },
  { title: '当前值', colKey: 'value', ellipsis: true },
  { title: '操作', colKey: 'op', width: 150, fixed: 'right' },
]);

const knownVars = KNOWN_CRS_VARS;

const crsVarRows = computed(() => Object.entries(crsVars.value).map(([key, value]) => ({ key, value })));
...
function gotoUrlWhitelist() {
  router.push({ name: 'WafUrlWhiteList' });
}

function gotoIpWhitelist() {
  router.push({ name: 'WafIpWhiteList' });
}
...
function getTuning() {
  owaspTuningGetApi().then((res) => {
    if (res.code === 0 && res.data) {
      const d = res.data;
      form.rule_engine = d.rule_engine || 'On';
      form.blocking_paranoia = d.blocking_paranoia_level || 1;
      form.detection_paranoia = d.detection_paranoia_level || 2;
      form.inbound_threshold = d.inbound_anomaly_score_threshold || 7;
      form.outbound_threshold = d.outbound_anomaly_score_threshold || 4;
      form.early_blocking = d.early_blocking || 0;
      form.request_body_limit = d.request_body_limit || 0;
      form.request_body_in_memory_limit = d.request_body_in_memory_limit || 0;
      form.body_inspect_limit = d.body_inspect_limit || 0;
      earlyBool.value = form.early_blocking === 1;
    }
  });
}
...
function onSubmit() {
  const payload = {
    rule_engine: form.rule_engine,
    blocking_paranoia_level: form.blocking_paranoia,
    detection_paranoia_level: form.detection_paranoia,
    inbound_anomaly_score_threshold: form.inbound_threshold,
    outbound_anomaly_score_threshold: form.outbound_threshold,
    early_blocking: form.early_blocking,
    request_body_limit: form.request_body_limit,
    request_body_in_memory_limit: form.request_body_in_memory_limit,
    body_inspect_limit: form.body_inspect_limit,
  };
...
function getCRSVars() {
  crsVarsLoading.value = true;
  owaspCRSVarsGetApi()
    .then((res) => {
      if (res.code === 0 && res.data) {
        crsVars.value = res.data.vars || {};
      }
    })
    .finally(() => {
      crsVarsLoading.value = false;
    });
}

function onAddVar() {
  Object.assign(editVarDialog, { visible: true, isNew: true, saving: false, key: '', value: '', placeholder: '', knownDesc: '' });
}

function onSetKnown(row: Record<string, any>) {
  const existing = crsVars.value[row.key] || '';
  Object.assign(editVarDialog, {
    visible: true,
    isNew: !existing,
    saving: false,
    key: row.key,
...
function onEditVar(row: Record<string, any>) {
...
function onSaveVar() {
  const key = (editVarDialog.key || '').trim().replace(/^tx\./, '');
  const { value } = editVarDialog;
  if (!key) {
    MessagePlugin.warning('变量名不能为空');
    return;
  }
  editVarDialog.saving = true;
  owaspCRSVarSetApi({ key, value })
    .then((res) => {
      if (res.code === 0) {
        MessagePlugin.success(res.msg || '已设置并热重载');
        editVarDialog.visible = false;
        getCRSVars();
      } else {
        MessagePlugin.error(res.msg || '设置失败');
      }
    })
    .catch(() => {
    ...
    .finally(() => {
      editVarDialog.saving = false;
    });
}

function onDeleteVar(key: string) {
  owaspCRSVarDeleteApi(key)
    .then((res) => {
      if (res.code === 0) {
        MessagePlugin.success('已删除并热重载');
        getCRSVars();
      } else {
