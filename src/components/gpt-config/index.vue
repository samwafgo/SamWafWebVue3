      ...
      <div class="gpt-preset-list">
        <div v-for="(p, pIndex) in gptProviders" :key="pIndex" class="gpt-preset-item">
          ...
          <div class="gpt-preset-ops">
            ...
            <t-link theme="primary" hover="color" size="small" :href="p.home" target="_blank">{{
              t('page.gpt.config.preset_apply_key')
            }}</t-link>
          </div>
        </div>
      </div>
...
const emit = defineEmits<{
  ...
  (e: 'saved', payload: { has_token: boolean }): void;
}>();
...
watch(
  () => props.visible,
  (val) => {
    if (val) loadGptConfig();
  },
);

onMounted(() => {
  if (props.visible) loadGptConfig();
});

/** 预设服务商说明（按界面语言） */
function gptPresetDesc(p: any) {
  const en = String(locale.value || '').toLowerCase().indexOf('en') === 0;
  return en ? p.en : p.zh;
}

/** 应用预设：填 url + 选定/默认模型 */
function applyGptPreset(p: any, model?: string) {
...
async function saveGptConfig() {
  ...
  try {
    ...
    if (res.code === 0) {
      ...
      // 通知调用方刷新"是否已配置"状态
      emit('saved', { has_token: gptConfig.has_token });
    } else {
      MessagePlugin.warning(res.msg || t('page.gpt.config.save_fail'));
    }
  } catch (e) {
    console.log(e);
    MessagePlugin.error(t('page.gpt.config.save_fail'));
  } finally {
    saving.value = false;
  }
}
